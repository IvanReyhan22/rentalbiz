    ...
    Scaffold(
        ...
        Box(modifier = modifier.padding(paddingValues)) {
            when (uiState) {
                ...
                is UiState.Success -> {
                    FavoriteContent(
                        data = (uiState as UiState.Success<List<FavoriteItem>>).data,
                        onItemClick = {
                        ...
                        onFavoriteClick = { viewModel.deleteProduct(it) }
                    )
                }
                else -> ScreenState(isLoading = true)
            }
        }
    }
...
private fun ScreenState(
    ...
    Column(
        ...
        if (isLoading) {
            ...
            Heading(
                title = stringResource(id = R.string.favorite_empty),
                type = HeadingType.H5,
...
fun FavoriteScreenPreview() {
    RentalBizTheme {
        FavoriteContent(data = emptyList(), onItemClick = {}, onFavoriteClick = {})
    }
}